...
# we COULD make it an expression, but the DSL would still only be able to use it as a statement
...
class Function(CoreNode):
    ...
    def __init__(self, functionName: str, functionParams: list[Variable] | None = [], functionBody: Block | None = None):
        super().__init__()
...
class If(Control):
    ...
    def __init__(self, condition: Condition, then: Block | None = None, elseifs: list[tuple[Condition, Block]] = [], elsethen: Block | None = None):
        super().__init__()
...
class Command(Expression):
    ...
    def __init__(self, command: Expression | None = None, args: list[Expression] | None = [], name: str | None = None):
        super().__init__()
        self.command = command
        self.args = args
...
# real sugarNodes should be handled in Prepass phase, or another sugarnode-specific phase
...
# associates nested blocks, performs variable renaming, detects when "capture" function needs to be injected
class PrePass:
    ...
    def _(self, node: Return, parent: Node):
        assert(isinstance(parent, Block) and parent.parent != None)
    ...
    def _(self, node: Variable, parent: Node):
        ...
        if existingVariable:
            ...
            self.environment.setVariable(node.name, node) #both names now refer to variable
            return node
...
class ShellRenderer:
    ...
    def _(self, node: Return):
        ...
        if node.returnMethod == 'echo':
            ...
            raise NotImplementedError(f'Error, returnMethod of type {node.returnMethod} is not supported')
    ...
    def _(self, node: Print) -> str:
        args = f'{' '.join([f"$(printf {self.visit(arg)})" for arg in (node.args or [])])}'
    ...
    def _(self, node: Command):
        ...
        if node.defined_stdout and node.defined_stderr:
            ...
            text += f'{self.visit(node.command)}{args} >/dev/null 2>&1'
